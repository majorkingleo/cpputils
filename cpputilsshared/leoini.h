...
public:
	struct Element ///< This structure represents an ini-file entry
	{
		...
		struct EnumType {
			enum ETYPE {
				FIRST__, SECTION_LIST, ///< the element is a section list
...
private:
	...
	std::ios_base::openmode openmode; ///< mode how we opened the file
...
public:
	...
	Ini(std::string filename, std::ios_base::openmode mode = std::ios_base::in | std::ios_base::out);
	...
	bool open(std::string filename, std::ios_base::openmode mode = std::ios_base::in | std::ios_base::out);
